...
L1 and L3 are the two best regression models. The Best regression model is important. 
...
as.data.frame(summary.out$outmat)
library(car)
subsets(regsubsets.out,statistic="adjr2",legend="right",main="Adjusted R^2")
```
...
We used two regressor models to find the r squared for pike, perch, and roach. Roach had the lowest r squared value in comparison to all the other fish. The adjusted r squared for roach was 0.8281, the next smallest was perch which is 0.94, then the largest adjusted r squared is 0.9794. 